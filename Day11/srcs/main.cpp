#include "../inc/main.hpp"
#include "../inc/FileParsing.hpp"
#include "../inc/Solution.hpp"

const char *input = "map.txt";
const char *inputExample = "map_example.txt";
const char *inputExampleBonus = "map_bonus_example.txt";

int main() {
	FileParsing fpExampleBonus = FileParsing(inputExampleBonus);
	FileParsing fpExample = FileParsing(inputExample);
	FileParsing fp = FileParsing(input);
	/*
	Get input --- Instead of a vector i can have a unique string if the input is a single line
	std::string contentExample = fpExample.getContent();
	std::string content = fp.getContent();
	*/
	std::unordered_map<std::string, std::vector<std::string>> contentExampleBonus = fpExampleBonus.getContent();
	std::unordered_map<std::string, std::vector<std::string>> contentExample = fpExample.getContent();
	std::unordered_map<std::string, std::vector<std::string>> content = fp.getContent();

	/* Load Solution Class */
	Solution solutionExampleBonus = Solution(contentExampleBonus);
	Solution solutionExample = Solution(contentExample);
	Solution solution = Solution(content);

	solutionExample.run();
	solutionExampleBonus.runBonus();
	solutionExample.showResolution(true, false);
	solutionExampleBonus.showResolution(false, true);

	auto start = std::chrono::high_resolution_clock::now();
	solution.run();
	auto end = std::chrono::high_resolution_clock::now();
	solution.runBonus();
	auto endBonus = std::chrono::high_resolution_clock::now();
	solution.showResolution(true, true);
	auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
	auto durationBonus = std::chrono::duration_cast<std::chrono::milliseconds>(endBonus - end);
	std::cout << "The level 1 duration was " << duration.count() << "ms" << std::endl;
	std::cout << "The level 2 duration was " << durationBonus.count() << "ms" << std::endl;
	
	return (0);
}
